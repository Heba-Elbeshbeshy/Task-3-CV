...
def detectAndCompute(image, sigma=1.6, num_intervals=3, assumed_blur=0.5, image_border_width=5):
    ...
    # keypoints_with_orientations = computeKeypointsWithOrientations(R, num_octaves, gaussian_images)
...
if __name__ == '__main__':
    img = cv2.imread('deer_orignal.jpg')
    gray= cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    gray = np.float32(gray)
    R = cv2.cornerHarris(gray, 2, 3, 0.04)
    print(R)
    t1 = time.time()
    kp, dc = detectAndCompute(gray)
    t2 = time.time()
    print(kp, dc)
